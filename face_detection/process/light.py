...
def compensate_light(image: np.ndarray, ycc: np.ndarray | None = None):
    ...
    scale = 1 if image.dtype == np.float32 else 255
    ...
    reference_white = image[reference_white_pixels].mean(axis=0) / scale

    # Escalona cada canal de cor para que o branco de referência (top 5%) seja (255, 255, 255)
    lux_corrected_image = (image * (1 + (1 - reference_white))).clip(0, scale).astype(image.dtype)
